...
// Location map images go into nocropi map tool/locations/
const LOCATIONS_IMG_DIR = path.join(__dirname, 'nocropi map tool', 'locations');
...
const WATCH_DIRS = [__dirname, path.join(__dirname, 'nocropi map tool'), path.join(__dirname, 'npcs')];
...
function deleteImageFile(imageUrl) {
  ...
  // Try both the web root and the nocropi map tool subfolder (for location images)
  const allowed = __dirname + path.sep;
  [
    path.join(__dirname, rel),
    path.join(__dirname, 'nocropi map tool', rel)
  ].forEach(full => {
...
app.use((req, res, next) => {
  if (/^\/content\b/i.test(req.path) || /^\/nocropi map tool\/content\b/i.test(req.path))
    return res.status(403).end();
  next();
});
...
app.use(express.static(path.join(__dirname, 'nocropi map tool'), { setHeaders: imgHeaders }));
